...
contract zkMedDomainProverTest is VTest {
    ...
    function test_verifyHospitalRegistration() public {
        ...
        // Call the prover
        callProver();
        (, zkMedDomainProver.RegistrationData memory regData) = prover.proveOrganizationDomain(email);

        // Verify the extracted data matches expected values
        assertEq(uint256(regData.requestedRole), uint256(zkMedDomainProver.UserType.HOSPITAL));
    ...
    function test_verifyInsurerRegistration() public {
        ...
        // Verify the extracted data matches expected values
        assertEq(uint256(regData.requestedRole), uint256(zkMedDomainProver.UserType.INSURER));
    ...
    function test_verifyPatientRegistration() public {
        ...
        // Call the prover
        callProver();
        (, zkMedDomainProver.RegistrationData memory regData) = prover.provePatientEmail(email);
