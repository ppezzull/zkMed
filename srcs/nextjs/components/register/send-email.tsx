...
export default function SendEmail({ role, organizationName, onEmailSent, onBack }: SendEmailProps) {
  ...
  const getSubject = () => {
    ...
    switch (role) {
      case 'PATIENT':
        return `Register patient with wallet: ${account.address}`;
      case 'HOSPITAL':
      case 'INSURER':
        if (!organizationName) return '';
        return `Register organization ${organizationName} as ${role} with wallet: ${account.address}`;
      default:
        return '';
    }
  };
