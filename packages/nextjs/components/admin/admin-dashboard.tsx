...
export default function AdminDashboard({ initialData }: AdminDashboardProps) {
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      // Approve all selected requests (mock implementation)
  ...
  const handleApprove = async (_requestId: bigint) => {
    try {
      await admin.approveRequest();
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const handleReject = async (_requestId: bigint) => {
    ...
    try {
      await admin.rejectRequest(rejectReason);
    ...
    <div className="container mx-auto px-4 py-8">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">
              {initialData.registrationStats.totalPatients.toString()}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {initialData.registrationStats.totalHospitals.toString()}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">
              {initialData.registrationStats.totalInsurers.toString()}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Review and approve or reject registration requests</CardDescription>
        ...
        <CardContent>
          <Tabs defaultValue="organization" className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              ...
              <TabsTrigger value="admin">Admin Requests ({initialData.adminRequests.length})</TabsTrigger>
            </TabsList>
    ...
    const fetchRequestData = async () => {
      ...
      try {
        if (type === "organization") {
          ...
          const baseRequest = await admin.fetchRequestBase();
          setRequestData(baseRequest);
        } else {
          const adminRequest = await admin.fetchAdminRequest();
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                ...
                {requestData.requester
                  ? `${requestData.requester.slice(0, 6)}...${requestData.requester.slice(-4)}`
                  : "Unknown"}
              </p>
              {type === "admin" && requestData.reason && (
                <p className="text-sm text-gray-600 mt-1">Reason: {requestData.reason}</p>
              )}
              <p className="text-xs text-gray-500">
                Requested:{" "}
                {requestData.requestTime
                  ? new Date(Number(requestData.requestTime) * 1000).toISOString().split("T")[0]
                  : "Unknown"}
              </p>
            </div>
          </div>
